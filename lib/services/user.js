...
export class UserService {
  ...
  async getAllUsers() {
    try {
      this.log.info('Get All Users: Start')
      ...
      if (isEmpty(users)) {
        this.log.info('Get All Users: Not Found')
        return null
      }

      this.log.info('Get All Users: Success')

      return users
    } catch (err) {
      this.log.error({ err }, 'Get All Users: Fail')
      throw err
    }
  }
  ...
  async getUserById({ userId }) {
    try {
      this.log.info({ userId }, 'Get User By Id: Start')
      const users = await this.mongoService.fetch({
        query: { id: userId },
        collection: 'user'
      })
      ...
      this.log.info({ userId }, 'Get User By Id: Success')

      return head(users)
    } catch (err) {
      this.log.error({ err }, 'Get User By Id: Fail')
      throw err
    }
  }
  ...
  async resetPassword({ userId, password }) {
    try {
      this.log.info({ userId }, 'Reset Password: Start')
      const user = await this.mongoService.fetch({
        query: { userId },
        collection: 'user'
      })
      if (!user) {
        this.log.info({ userId }, 'Get User: Not Found')
      ...
      this.log.info({ userId }, 'Reset Password: Success')
  ...
  async purchase({ email, amount, token }) {
    try {
      ...
      const chargeResp = await this.transactionService.charge({ amount, token })
